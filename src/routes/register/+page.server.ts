...
export async function load({url, request}) {
  ...
  if (flow != null) {
    const response = await fetch(`${authURL}/self-service/registration/flows?id=${flow}`, {
      headers: {
  ...
  const response = await fetch(`${authURL}/self-service/registration/browser`, {
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
      "Cookie": request.headers.get('cookie') || ''
  ...
  if (response.ok) {
    return redirect(303, `/register?flow=${(await response.json()).id}`)
  }
